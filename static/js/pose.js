...
const EXERCISES = {
  ...
  'Lateral Raise': {
    ...
    extended: 85, contracted: 15,
    cues: { top: 'Shoulder height, nice!', bottom: 'Controlled lowering.' },
    formCheck: (kp) => checkTorsoLean(kp),
  },
  'Front Raise': {
    joints: ['left_hip', 'left_shoulder', 'left_elbow'],
    extended: 88, contracted: 15,
    cues: { top: 'Nice raise!', bottom: 'Controlled, good.' },
    formCheck: (kp) => checkTorsoLean(kp),
  },
  'Barbell Curl': {
    joints: ['left_shoulder', 'left_elbow', 'left_wrist'],
    extended: 155, contracted: 60,
    cues: { top: 'Great squeeze!', bottom: 'Full extension.' },
    formCheck: (kp) => checkElbowDrift(kp, 'left_shoulder', 'left_elbow'),
  },
  'Hammer Curl': {
    joints: ['left_shoulder', 'left_elbow', 'left_wrist'],
  ...
  'Tricep Pushdown': {
    joints: ['left_shoulder', 'left_elbow', 'left_wrist'],
    extended: 165, contracted: 80,
    cues: { top: 'Full extension!', bottom: 'Controlled return.' },
    formCheck: (kp) => checkElbowDrift(kp, 'left_shoulder', 'left_elbow'),
  },
  'Overhead Tricep Extension': {
    joints: ['left_shoulder', 'left_elbow', 'left_wrist'],
    extended: 165, contracted: 65,
    cues: { top: 'Full lockout!', bottom: 'Good stretch.' },
    formCheck: (kp) => checkElbowDrift(kp, 'left_shoulder', 'left_elbow'),
  },
  'Lunges': {
    joints: ['left_hip', 'left_knee', 'left_ankle'],
...
function checkTorsoLean(kp) {
  if (!kp.left_shoulder || !kp.left_hip) return { ok: true };
  const lean = Math.abs(kp.left_shoulder.x - kp.left_hip.x);
  if (lean > 40) return { ok: false, msg: 'Keep your torso still, no swinging the weight up.' };
  return { ok: true };
}
...
function stopCamera() {
  ...
  if (identityCheckTimer) { clearInterval(identityCheckTimer); identityCheckTimer = null; }
}

// --------------------------------------------------------------------
// Continuous identity check during the workout (optional — only runs if
// the signed-in user enrolled Face ID). Reuses the same webcam <video>
// element already used for pose detection; only a numeric face descriptor
// is ever computed, nothing is uploaded as an image, and matching against
// the person's own stored descriptor happens right here in the browser.
// --------------------------------------------------------------------
...
let identityPausedForMismatch = false;

async function loadFaceModelsIfNeeded() {
  if (faceModelsReady) return true;
  if (typeof faceapi === 'undefined') return false;
  for (let i = 0; i < FACE_MODEL_URLS.length; i++) {
    try {
      await faceapi.nets.tinyFaceDetector.loadFromUri(FACE_MODEL_URLS[i]);
      await faceapi.nets.faceLandmark68Net.loadFromUri(FACE_MODEL_URLS[i]);
      await faceapi.nets.faceRecognitionNet.loadFromUri(FACE_MODEL_URLS[i]);
      faceModelsReady = true;
      return true;
    } catch (e) {
      console.warn(`Face ID model source ${i + 1} failed:`, e);
    }
  }
  console.warn('Face ID models failed to load from every source — identity check disabled this session.');
  return false;
}

async function initIdentityCheck(user) {
  try {
    const res = await api('/api/face/my-descriptor');
    if (!res.descriptor) return; // never enrolled Face ID — skip silently, feature is optional
    myFaceDescriptor = res.descriptor;
    myFaceName = res.name || user.name;
  } catch (e) {
    return;
  }
  $('hudIdentityWrap').style.display = 'flex';
  $('hudIdentity').textContent = 'Loading Face ID…';
  const ready = await loadFaceModelsIfNeeded();
  if (!ready) {
    $('hudIdentity').textContent = '⚠️ Face ID unavailable';
    setTimeout(() => { $('hudIdentityWrap').style.display = 'none'; }, 4000);
    return;
  }
  $('hudIdentity').textContent = 'Watching for you';
}

async function runIdentityCheck() {
  if (!running || paused || !myFaceDescriptor || !faceModelsReady || identityPausedForMismatch) return;
  ...
  try {
    ...
    if (!result) {
      ...
      const dist = faceapi.euclideanDistance(result.descriptor, myFaceDescriptor);
      if (dist <= FACE_MATCH_THRESHOLD) {
        identityMismatchStreak = 0;
        $('hudIdentity').textContent = `✅ ${myFaceName}`;
      } else {
        identityMismatchStreak += 1;
      }
    }
    // Require two consecutive misses (~30s apart) before interrupting, so a
    // single bad angle or a hand over the face doesn't falsely flag someone.
    if (identityMismatchStreak >= 2) triggerIdentityMismatch();
  } catch (e) { /* skip this cycle on any detection error, try again next interval */ }
}

function triggerIdentityMismatch() {
  identityPausedForMismatch = true;
  paused = true;
  $('pauseBtn').textContent = '▶ Resume';
  $('hudIdentity').textContent = '⚠️ Face not recognized';
  $('identityMismatchTitle').textContent = `⚠️ Face not recognized`;
  $('identityMismatchBody').textContent =
    `This doesn't look like ${myFaceName}. Your workout has been stopped and won't ` +
    `continue until this is resolved. If that's wrong, tap "It's still me" — otherwise, ` +
    `please log in as yourself to keep your workout data accurate.`;
  $('identityMismatchOverlay').classList.add('active');
  speak(`Face not recognized. This doesn't look like ${myFaceName}. Your workout has been stopped.`);
}

function dismissIdentityMismatch(confirmedSelf) {
  $('identityMismatchOverlay').classList.remove('active');
  identityPausedForMismatch = false;
  identityMismatchStreak = 0;
  if (confirmedSelf) {
    paused = false;
    $('pauseBtn').textContent = '⏸ Pause';
    $('hudIdentity').textContent = `✅ ${myFaceName}`;
    speak('Welcome back — resuming.');
  }
}
...
(async function initWorkoutPage() {
  ...
  await loadQueue();
  await initIdentityCheck(user);
  // Checks every 6s; two consecutive misses (~12s) trigger a stop — fast
  // enough to catch a face swap quickly, while still ignoring one bad frame
  // (a hand passing by, a bad angle) so it doesn't falsely interrupt a real set.
  identityCheckTimer = setInterval(runIdentityCheck, 6000);
  ...
  $('pauseBtn').addEventListener('click', () => {
    if (identityPausedForMismatch) return; // ignore manual pause toggle while an identity check is pending
    paused = !paused;
  ...
  $('identityStillMeBtn').addEventListener('click', () => dismissIdentityMismatch(true));
  $('identityLogoutBtn').addEventListener('click', async () => {
    dismissIdentityMismatch(false);
    stopCamera();
